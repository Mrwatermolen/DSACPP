...
template <typename Tv> struct Vertex {
  ...
  Vertex()
      : inDegree(0), outDegree(0), status(UNDISCOVERED), dTime(-1), fTime(-1),
...
template <typename Tv, typename Te> struct DjikPu {
  virtual void operator()(MyGraph<Tv, Te> *g, int uk, int v) {
    if (g->Status(v) != UNDISCOVERED) {
      return;
    }
...
/// @brief 复位所有顶点和边的信息
template <typename Tv, typename Te> void MyGraph<Tv, Te>::reset() {
  for (int i = 0; i < n; i++) {
...
template <typename Tv, typename Te> void MyGraph<Tv, Te>::BFS(int start) {
  reset();
...
/// @brief 获得图的DFS序列
/// @param start 以顶点start为起点
template <typename Tv, typename Te> void MyGraph<Tv, Te>::DFS(int start) {
  reset();
  int clock = 0, v = start;
  while (true) {
    if (Status(v) == UNDISCOVERED) {
...
/// @return
template <typename Tv, typename Te> MyBitmap MyGraph<Tv, Te>::BBC() {
  MyBitmap B(n);
  srand(time(0));
  int clock = 0;
  MyStack<int> S;
